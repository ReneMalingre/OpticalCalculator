      ...
      if (overrefraction.checkValidity()) {
        ...
        if (vertexdistance.checkValidity()) {
          ...
          if (newbase.checkValidity()) {
            ...
            const ocularRefraction =
              overRefractionValue /
              (1 - (vertexDistanceValue / 1000) * overRefractionValue);

            const refractiveIndexTears = 1.336;
            const frontSurfaceTearPowerExisting =
              ((refractiveIndexTears - 1) * 1000) / existingBaseValue;
            const frontSurfaceTearPowerNew =
              ((refractiveIndexTears - 1) * 1000) / newBaseValue;
            const changeInFrontSurfaceTearPower =
              frontSurfaceTearPowerNew - frontSurfaceTearPowerExisting;
            const newPowerValue =
              Math.round(
                (-1 * changeInFrontSurfaceTearPower +
                  ocularRefraction +
                  existingPowerValue) *
                  100
              ) / 100;
            ...
            document.getElementById("result").innerHTML =
              "The new power will be " + formattedNumber + " D";
          } else {
            document.getElementById("result").innerHTML =
              "Your new base curve is not valid";
          }
        } else {
          document.getElementById("result").innerHTML =
            "Your vertex distance is not valid";
        }
      } else {
        document.getElementById("result").innerHTML =
          "Your over-refraction is not valid";
      }
    } else {
      document.getElementById("result").innerHTML =
        "Your existing power is not valid";
    }
  } else {
    document.getElementById("result").innerHTML =
      "Your existing base curve is not valid";
  }
}
// Get the modal
